BEGIN
  ...
  OPEN empresas;
	...
	FETCH empresas INTO id_emp;
		...
		DECLARE subgrupo VARCHAR(255);
		DECLARE productos CURSOR FOR select id,empresa,grupo,subgrupo from agil.agil_producto where empresa=id_emp GROUP BY grupo;
		DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

		OPEN productos;

		read_loop: LOOP
			FETCH productos INTO id_producto,id_empresa,grupog,subgrupo;
			IF done THEN
				LEAVE read_loop;
			END IF;
			...
			IF count_clase=0 THEN
				BLOCK4: BEGIN
					...
					DECLARE id_tipo INT;
					DECLARE tipoDato CURSOR FOR select id from agil.gl_tipo where agil.gl_tipo.nombre=CONVERT("GRUPOS PRODUCTOS" using utf8) collate utf8_spanish_ci and agil.gl_tipo.empresa=id_empresa;
					...
					readtipoDato_loop: LOOP
						...
						INSERT INTO agil.gl_clase SET tipo=id_tipo, nombre=grupog, nombre_corto='GRUPO_PRODUCTO',createdAt=Now(),updatedAt=Now();
