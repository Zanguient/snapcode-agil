angular.module('agil.controladores')
	.controller('ControladorEstadoCuentasProveedores', ['$scope', '$localStorage', '$location', '$templateCache', '$route', 'blockUI', '$timeout',
		'uiGmapGoogleMapApi', '$cordovaGeolocation', 'ReportEstadoCuentasProveedoresDatos', 'InventariosCosto','ObtenerAnticiposProveedor','PagosCompraCreditosAnticipo',function ($scope, $localStorage, $location, $templateCache, $route, blockUI, $timeout,
		uiGmapGoogleMapApi, $cordovaGeolocation, ReportEstadoCuentasProveedoresDatos, InventariosCosto,ObtenerAnticiposProveedor,PagosCompraCreditosAnticipo) {
		blockUI.start();
	...
	.controller('ControladorEstadoCuentasClientes', ['$scope', '$window', '$localStorage', '$location',
		...
		'ObtenerAnticiposCliente','PagosVentaCreditosAnticipo',function ($scope, $window, $localStorage, $location,
		$templateCache, $route, blockUI, $timeout, Paginator,
		uiGmapGoogleMapApi, $cordovaGeolocation, ReportEstadoCuentasClientesDatos, InventariosCosto, ReporteClientesPaginador,
		ObtenerAnticiposCliente,PagosVentaCreditosAnticipo) {
		blockUI.start();
		$scope.idModalTablaEstadoCuenta = "tabla-estado-cuenta";
		...
		$scope.imprimirRecibo = function (pagosVenta, venta) {
			...
			doc.moveDown(0.5);
			doc.font('Helvetica-Bold', 8);
			if(pagosVenta.anticipos){doc.text("COMPENSACION", { align: 'center' });}else{doc.text("RECIBO", { align: 'center' });}
			doc.font('Helvetica', 7);
			doc.moveDown(0.4);
			doc.text("------------------------------------", { align: 'center' });
			doc.moveDown(0.4);
		...
		$scope.$on('$routeChangeStart', function (next, current) {
			$scope.eliminarPopup($scope.idModalTablaEstadoCuenta);
			$scope.eliminarPopup($scope.idModalPagoDeudaCliente);
		});
		$scope.inicio();
	}])
