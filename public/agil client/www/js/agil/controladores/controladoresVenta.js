		...
		'GuardarImportacionVentasFacturacion','GuardarImportacionPagosVenta', function ($scope, $filter, $localStorage, $location, $templateCache, $route, blockUI, $timeout, $window, InventarioPaginador,
							...
							if (detalleVenta.costos.length > 1) {
								...
								if (detalleVenta.descuento > 0) {
									if (detalleVenta.tipo_descuento) {
										$scope.venta.total_descuento_general += (((detalleVenta.precio_unitario * detalleVenta.descuento) / 100) * detalleVenta.cantidad)
									} else {
			...
			$scope.crearNuevaVenta = function (venta) {
				...
				$scope.venta = new Venta({
					...
					detallesVenta: [], detallesVentaNoConsolidadas: [], pagado: 0, cambio: 0, despachado: false, vendedor: null, total_descuento_general: 0
				});
				var al = 0;
				if (venta == undefined) {
						...
						do {
							...
							do {
								...
								if (NumeroCompraA == venta.factura) {
									var detalleVenta = { producto: {}, centroCosto: {}, cantidad: 1, descuento: 0, recargo: 0, ice: 0, excento: 0, tipo_descuento: false, tipo_recargo: false, eliminado: false }
